...
- **Flag name-matched benchmarks** *(v1.18.0 · PROVISIONAL · off by default)* — marks every multi-quote benchmark whose quotes do not all share one part number as **unverified**. Nothing is re-matched and the median is unchanged; the cluster simply loses its reliable spread (amber `*`, "Unverified" wording in every drill-down and export) and Assess a Claim applies no statistical bound to it. The basis is Claude's first-pass labelling of the live gold set (`eval/README.md`): about 27% of name-only merges joined the same part, mostly failing on the same part name across vehicles. That has **not** been verified by a claims adjuster, hence opt-in. It appears in both the Configuration tab and the Benchmark tab's matching panel, including Simple mode. When on it is written to the detailed report's Summary and hashed into the snapshot id; off leaves every snapshot id exactly as before.
...
**Claim History persists the same way (added v1.16.0).** Assess a Claim's
**Save to claim history** button writes a record — the full assessed lines
(including match evidence, so it can still be re-exported), the matching-config
snapshot it ran under, and any workshop name / vehicle plate / make / model an
OCR read of the estimate found — through the same backend switch:
`loadClaims()` / `saveClaim()` / `deleteClaim()` in `src/datasource.js` go to a
`localStorage` key (`partsindex_claims_v1`, capped at the 200 most recent) by
default, or to a new `claims` table via `GET`/`POST`/`DELETE /api/claims`
(served by `api/claims.js`, backed by `getClaims` / `saveClaim` / `deleteClaim`
in `api/_db.js`) when `VITE_DATA_BACKEND=api`. Adding the table bumped
`SCHEMA_VERSION` to 3; like the activity table before it, `CREATE TABLE IF NOT
EXISTS` means re-running `ensureSchema()` (every request does, cheaply) against
an existing database only adds the new table and never touches `parts` or
`activity`. On the shared backend, every reviewer sees the same claim history.
...
> **New features** beyond fixing limitations — ~~estimate OCR intake~~ (**done**
> in v1.14.0),
...
### Worklist is not saved

The Benchmark tab's worklist (v1.8.0) is held in React state inside the tab, so
it is lost whenever the tab unmounts — switching tab, reloading, or toggling
Simple/Detailed — with no warning. Nothing in the UI says so. Persisting it
(browser storage, as the Simple/Detailed choice already is) or at least warning
before it is dropped would close the gap; neither is scheduled.
