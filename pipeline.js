...
/* ---- dispersion measures (IQR, SD, CV) ----
   Deliberately Excel-consistent so figures reconcile against a spreadsheet of
   the same quotes: quantiles use R-7 / PERCENTILE.INC (linear interpolation),
   and stdev is the SAMPLE deviation (n-1), matching STDEV.S. */
// R-7 / Excel PERCENTILE.INC on an ascending-sorted copy.
export function quantile(a, p) {
  const s = [...a].sort((x, y) => x - y); const n = s.length;
  if (!n) return NaN; if (n === 1) return s[0];
  const rank = p * (n - 1), lo = Math.floor(rank), frac = rank - lo;
  return frac === 0 ? s[lo] : s[lo] + frac * (s[lo + 1] - s[lo]);
}
// Sample standard deviation (n-1). NaN for n<2 on purpose: a single quote has no
// measurable spread and must NOT read as zero variance / perfect agreement.
export function stdev(a) {
  const n = a.length; if (n < 2) return NaN;
  const m = mean(a); let ss = 0; for (const x of a) ss += (x - m) ** 2;
  return Math.sqrt(ss / (n - 1));
}
/* One-pass dispersion summary for a group of unit prices.
   minN = observation floor below which spread is only advisory (reliable:false).
   Coefficient of variation (SD/mean, %) is reported instead of raw SD so spread
   is comparable across a $40 clip and a $4,000 bumper. Tukey fences (Q±1.5·IQR)
   drive statistically defensible outlier flagging in claim assessment. */
export function dispersion(units, minN = 4) {
  const n = units.length; if (!n) return null;
  const q1 = quantile(units, 0.25), q3 = quantile(units, 0.75);
  const iqr = q3 - q1, sd = stdev(units), m = mean(units);
  const cv = Number.isFinite(sd) && m > 0 ? +((sd / m) * 100).toFixed(1) : NaN;
  return {
    q1: +q1.toFixed(2), q3: +q3.toFixed(2), iqr: +iqr.toFixed(2),
    sd: Number.isFinite(sd) ? +sd.toFixed(2) : NaN, cv,
    lowerFence: +(q1 - 1.5 * iqr).toFixed(2), upperFence: +(q3 + 1.5 * iqr).toFixed(2),
    reliable: n >= minN,
  };
}
...
export function buildClusters(parts, cfg) {
  ...
  if (cfg.bridge) {
    ...
    return merged.map((mem) => makeCluster(mem, cfg)).sort((a, b) => b.n - a.n);
  }
  return groups.map((g) => makeCluster(g.mem, cfg)).sort((a, b) => b.n - a.n);
}
...
export function fuzzyAgglomerate(usable, cfg) {
  ...
  for (let i = 0; i < usable.length; i++) {
    ...
    clusters.push(makeCluster(mem, cfg));
  }
  return clusters;
}

export function makeCluster(mem, cfg = {}) {
...
export function buildDisputePack(rows, cfg, meta) {
  ...
  const summary = [
    ...
    ["Lines above statistical bound (Q3 + 1.5·IQR)", rows.filter((r) => r.aboveFence).length],
  ...
  const lines = rows.map((r, i) => ({
    ...
    "IQR band S$": r.cluster ? `${r.cluster.q1}-${r.cluster.q3}` : "",
    "Stat. upper bound S$": r.cluster && r.cluster.reliable && Number.isFinite(r.cluster.upperFence) ? r.cluster.upperFence : "",
    "Above stat. bound": r.aboveFence ? "YES" : "",
