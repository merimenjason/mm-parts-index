...
- **Grade, GST & unit-basis fields** — every line carries a parts **grade**
  ...
  — never guessed.
- **Flag name-matched benchmarks** *(v1.18.0, PROVISIONAL, off by default)* — on the
  Configuration tab and in the Benchmark tab's matching panel (Simple mode too). When
  on, any benchmark whose quotes do not all share one part number — grouped by name
  only — is marked **unverified**: the median still shows, but its spread is advisory
  (amber `*`) and Assess a Claim applies no statistical bound. Motivated by a first-pass
  labelling of live part pairs (by Claude, not yet adjuster-verified) that found most
  name-only merges join different parts. On the live reference it leaves 3 of the 45
  4+-quote benchmarks verified. Stated in the detailed report and hashed into the
  snapshot id when on; off leaves every snapshot id unchanged.
